...
class Block {
    ...
    static serialize(inputBlock) {
        ...
        // Nonce
        // blockHexStr += inputBlock.nonce.toString(16).padStart(10, "0");
        // Tx root
    ...
    static deserialize(block) {
        ...
        // myBlock.nonce = parseInt(blockHexStr.slice(0, 10), 16);
        // blockHexStr = blockHexStr.slice(10);
        ...
        while (blockHexStr.length > 0) {
            ...
            myBlock.transactions.push([...Buffer.from(blockHexStr.slice(0, offset * 2), "hex")]);
    ...
    static async verifyTransactionAndTransit(myBlock, stateDB, codeDB, enableLogging = false) {        
        // Basic verification
        ...
        const addressesInBlock = myBlock.transactions.map(tx => SHA256(Transaction.getPubKey(tx)));
        ...
        for (const tx of myBlock.transactions) {
            ...
            if (!states[transactionSenderAddress]) {
                ...
                states[transactionSenderAddress].balance = (BigInt(senderState.balance) - totalAmountToPay).toString();
            } else {
            ...
            if (
                ...
                states[transactionSenderAddress].codeHash = SHA256(tx.additionalData.scBody);
                code[states[transactionSenderAddress].codeHash] = tx.additionalData.scBody;
            }
            ...
            if (states[tx.recipient].codeHash !== EMPTY_HASH) {
                ...
                const [ newState, newStorage ] = await jelscript(code[states[tx.recipient].codeHash], states, BigInt(tx.additionalData.contractGas || 0), stateDB, myBlock, tx, contractInfo, enableLogging);
        ...
        if (!existedAddresses.includes(myBlock.coinbase) && !states[myBlock.coinbase]) {
            states[myBlock.coinbase] = { balance: "0", codeHash: EMPTY_HASH, nonce: 0, storageRoot: EMPTY_HASH }
            code[EMPTY_HASH] = "";
        }
    
        if (existedAddresses.includes(myBlock.coinbase) && !states[myBlock.coinbase]) {
            states[myBlock.coinbase] = deserializeState(await stateDB.get(myBlock.coinbase));
            code[states[myBlock.coinbase].codeHash] = await codeDB.get(states[myBlock.coinbase].codeHash);
        }

        let gas = 0n;

        for (const tx of myBlock.transactions) { gas += BigInt(tx.gas) + BigInt(tx.additionalData.contractGas || 0) }

        states[myBlock.coinbase].balance = (BigInt(states[myBlock.coinbase].balance) + BigInt(BLOCK_REWARD) + gas).toString();
