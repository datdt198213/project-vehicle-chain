...
async function addTransaction(transaction, chainInfo, stateDB) {
  ...
  //   console.log(
  //     `\x1b[31mERROR\x1b[0m [${new Date().toISOString()}] Failed to add one transaction to pool: Transaction has nonce lower than nonce in pool.`
  //   );
...
async function clearDepreciatedTxns(chainInfo, stateDB) {
  ...
  for (const tx of txPool) {
    ...
    // Weak-checking
    if (
      Transaction.isValid(tx, stateDB) &&
      tx.nonce - 1 === maxNonce[txSenderAddress]
    ) {
      newTxPool.push(tx);
